...
from extractors.arcface_extractor import ArcFaceExtractor
...
class SimilarityPredictor:
    ...
    def __init__(self, model_path, extractor: BaseExtractor = ArcFaceExtractor(), threshold: float = 0.6, align=True):
        super().__init__()
        ...
        if os.path.exists(model_path):
            self.reload()
            return
        classes = os.listdir(FilePathManager.resolve("face_recognition/faces"))
        self.classes = {}
        for clz in classes:
            path = FilePathManager.resolve(f"face_recognition/faces/{clz}")
