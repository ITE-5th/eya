...
class OpenfacePredictor(Predictor):
    def __init__(self):
        ...
        self.net = openface.TorchNeuralNet(FilePathManager.resolve("face_recognition/data/nn4.small2.v1.t7"), 224, cuda=True)
        classes = os.listdir(FilePathManager.resolve("face_recognition/data/custom_images"))
        t = {}
        for clz in classes:
            path = FilePathManager.resolve(f"face_recognition/data/custom_images/{clz}")
